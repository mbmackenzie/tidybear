...
def test_select_single_col(df: DataFrame) -> None:
    assert_frame_equal(select(df, "A"), df.loc[:, ["A"]])


def test_select_multiple_col(df: DataFrame) -> None:
    assert_frame_equal(select(df, "A", "B"), df.loc[:, ["A", "B"]])


def test_select_kwarg_col(df: DataFrame) -> None:
...
def test_select_args_and_kwargs(df: DataFrame) -> None:
