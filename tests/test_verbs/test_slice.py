...
def test_slice_max_no_group(df: DataFrame) -> None:
    for n in range(1, 9):
        top_rows = slice_max(df, order_by="B", n=n)
        assert top_rows.B.tolist() == list(range(8, 8 - n, -1))


def test_slice_max_yes_group(df: DataFrame) -> None:
    for n in range(1, 5):
...
def test_slice_min_no_group(df: DataFrame) -> None:
    for n in range(1, 9):
        bottom_rows = slice_min(df, order_by="B", n=n)
        assert bottom_rows.B.tolist() == list(range(1, n + 1))


def test_slice_min_yes_group(df: DataFrame) -> None:
    for n in range(1, 5):
